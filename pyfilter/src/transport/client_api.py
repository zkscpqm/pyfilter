...
class TextFilterClient:
    ...
    def __init__(self, svc_channel: Channel, svc_client_stub: TextFilterServiceStub):
        """

        :param svc_channel:
        :param svc_client_stub:
        """
    ...
    @classmethod
    def new(cls, secure_port: int = None, insecure_port: int = None) -> 'TextFilterClient':
        """
        ...
        return TextFilterClient(channel, stub)
    ...
    def _get_client_stub(cls, secure_port: int = None, insecure_port: int = None) -> Tuple[Channel,
        ...
        if not secure_port and not insecure_port:
            raise ConnectionError("Please provide either a secure or insecure port!")
    ...
    def subscribe(self, callback: Callable) -> NoReturn:
        """
        Debug only: Subscribe to channel state changes and handle via a callback
        """
        self._channel.subscribe(callback, try_to_connect=True)

    def unsubscribe(self, callback: Callable) -> NoReturn:
        """
        Debug only: Unsubscribe from a channel state changes and handle via a callback
        """
